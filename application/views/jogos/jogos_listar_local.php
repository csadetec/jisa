<?php 
 ...
 // print_r($jogos);
  echo '</pre>';
  if($jogos):
    $this->table->set_heading('DATA', 'EQUIPES', 'PONTOS',  'VISUALIZAR');
    foreach($jogos as $j):
      $this->table->add_row(set_data($j->data).' - '.$j->horas_inicial,
       ...
       anchor('jogos/visualizar/'.$j->id_jogo, '<i class="fas fa-eye"></i>', array('title'=>'Visualizar Jogo'))
    );
    endforeach;
    $template = array(
